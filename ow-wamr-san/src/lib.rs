...
impl WasmRuntime for Wamr {
    ...
    fn run(
        ...
        wamr_reset_runtime();
        println!("wamr_reset_runtime: {}ns", time.elapsed().as_nanos());
...
pub fn wamr_run_module(
    ...
    unsafe {
        ...
        println!("wasm_runtime_load: {}ns", time.elapsed().as_nanos());
        ...
        println!("wasm_runtime_instantiate: {}ns", time.elapsed().as_nanos());
        ...
        println!(
            "wasm_runtime_create_exec_env: {}ns",
        ...
        println!(
            "wasm_runtime_lookup_wasi_start_function: {}ns",
            time.elapsed().as_nanos()
        );
        ...
        println!("call_function: {}ns", time.elapsed().as_nanos());
...
pub fn pass_string_arg(
    ...
    unsafe {
        if !wasm_runtime_copy_to_wasm(instance, memory_buffer_offset, json_bytes.len() as u64, json_bytes.as_ptr()) {
            return Err(anyhow::anyhow!("Failed to pass string args to WASM"));
        }
    }
...
fn get_return_value(
    ...
    let memory_buffer =
        unsafe { wasm_runtime_copy_pointer(instance, memory_buffer_offset, len as u64) as *const u8 };
